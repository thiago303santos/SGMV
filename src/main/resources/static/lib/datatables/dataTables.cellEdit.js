...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    ...
    jQuery.fn.extend({
        ...
        updateEditableCell: function (callingElement) {
            ...
            var blMoney = false;
            if (inputField[0].outerHTML.includes("mascaraMoedaDataTable")) {
                blMoney = true;
            }
            // Update
            ...
            function _update(newValue) {
                if (blMoney == true) {
                    newValue = formatNumberBanco(newValue);
                }
...
function getInputHtml(currentColumnIndex, settings, oldValue, callingElement) {
    ...
    switch (inputType) {
        ...
        case "datepicker-confirm":
            ...
            setTimeout(function () {
                ...
                if (inputSetting && inputSetting.options && typeof inputSetting.options.icon !== 'undefined') {
                    icon = inputSetting.options.icon;
                }

                var cell = $(callingElement).closest('td');
                if (cell.length > 0) {
                    var cellInput = cell.find('.datepick');
                    if (cellInput.length > 0) {
                        cellInput.datepicker({
                            showOn: "button",
                            buttonImage: icon,
                            ...
                            onSelect: function (dateText, inst) {
                                cellInput.updateEditableCell(cellInput);
                            }
                        }).datepicker("show");
                    } else {
                        console.error("Elemento .datepick não encontrado dentro da célula:", cell);
                    }
                } else {
                    console.error("Célula pai não encontrada para o elemento:", callingElement);
                }
            }, 100);
            break;
        case "text-confirm":
        ...
        case "number-confirm":
            ...
            break;
        case "money":
            input.html = startWrapperHtml + "<input id='ejbeatycelledit' type='text' class='" + inputCss + " text-right' onkeyup='mascaraMoedaDataTable(this)' onfocusout='$(this).updateEditableCell(this)' value='" + oldValue + "'></input>" + endWrapperHtml;
            break;
        default:
